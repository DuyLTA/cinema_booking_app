-- Seed complete booking sessions for the current calendar week.
-- Run this file in Supabase SQL Editor.
-- The script is idempotent: existing showtimes, prices, and seats are skipped.
-- It only creates sessions from now until the end of this week in Asia/Bangkok.
...
begin
  -- Monday of the current calendar week in the app timezone.
  v_week_start := (
    date_trunc('week', now() at time zone 'Asia/Bangkok')
  )::date;

  v_week_end := (v_week_start + interval '7 days')::date;
    ...
    loop
      for v_day_offset in 0..6 loop
        ...
        loop
          ...
          -- Do not create past sessions for this week.
          if v_start_at <= now() then
            continue;
          end if;

          if v_start_at >= (v_week_end at time zone 'Asia/Bangkok') then
            continue;
          end if;
